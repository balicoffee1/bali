...
# --- Тестовый номер для ревью в сторах и ручной проверки ---
# На этот номер SMS не отправляется, подходит только SMS_TEST_CODE.
# ВНИМАНИЕ: это намеренный обход подтверждения. Номер обязан оставаться
# заведомо несуществующим, а код — не совпадать с боевыми сценариями.
# Отключается SMS_TEST_LOGIN_ENABLED=False.
SMS_TEST_LOGIN_ENABLED = env.bool("SMS_TEST_LOGIN_ENABLED", default=True)
SMS_TEST_PHONE = env.str("SMS_TEST_PHONE", default="+77777777777")
SMS_TEST_CODE = env.str("SMS_TEST_CODE", default="0000")
