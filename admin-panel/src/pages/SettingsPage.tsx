    ...
    <div className="space-y-6 animate-fade-in font-montserrat">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <Tabs
          tabs={[
            ...
            { id: 'telegram', label: 'Подключить Telegram-бота', icon: <Send className="w-3.5 h-3.5 text-sky-500" /> },
        ...
        <div className="flex items-center gap-2">
          ...
          {activeSettingsTab === 'telegram' && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-r12 bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 font-semibold text-xs transition-colors"
            >
              <Send className="w-3.5 h-3.5 text-sky-500" />
              <span>Бот @happy_island_bot ↗</span>
            </a>
          )}
        </div>
      </div>
      ...
      {activeSettingsTab === 'telegram' && (
        <div className="space-y-6 animate-fade-in font-montserrat">
          ...
          <Card className="bg-gradient-to-r from-sky-500/10 via-sky-500/5 to-transparent border border-sky-200/80 p-6 space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-lg font-extrabold text-brand-dark">Telegram-бот сети Happy Island</h3>
                    <Badge variant="success" size="sm">
                      <CheckCircle2 className="w-3 h-3 mr-1 inline" />
                  ...
                  <p className="text-xs text-brand-dark-blue leading-relaxed max-w-2xl">
                    Бот отправляет мгновенные уведомления о новых отзывах гостей и заказах. Управляющие и бариста также могут просматривать текущую смену, аналитику дня и историю заказов прямо в чате с ботом.
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-sky-100">
              <div className="bg-white/80 backdrop-blur-xs p-3 rounded-r12 border border-sky-100 flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-sky-50 text-sky-600 flex items-center justify-center shrink-0">
                  <Store className="w-4 h-4" />
                </div>
                <div>
                  <span className="text-[10px] text-brand-gray-blue font-bold uppercase tracking-wider block">Всего кофеен</span>
                  <span className="text-base font-extrabold text-brand-dark">{shops.length}</span>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-xs p-3 rounded-r12 border border-sky-100 flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] text-brand-gray-blue font-bold uppercase tracking-wider block">Подключено точек</span>
                  <span className="text-base font-extrabold text-brand-dark">
                    {shops.filter(s => (s.telegram_recipients_count ?? (s.telegram_id ? 1 : 0)) > 0).length} из {shops.length}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-xs p-3 rounded-r12 border border-sky-100 flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] text-brand-gray-blue font-bold uppercase tracking-wider block">Всего получателей</span>
                  <span className="text-base font-extrabold text-brand-dark">
                    {shops.reduce((acc, s) => acc + (s.telegram_recipients_count ?? (s.telegram_id ? 1 : 0)), 0)} чел.
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Quick How-it-works instruction cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {
                step: '1',
                title: 'Выберите заведение',
                desc: 'Выберите нужную точку кофейни из списка ниже для настройки получателей.',
                icon: '📍',
              },
              {
                step: '2',
                title: 'Создайте ссылку',
                desc: 'Нажмите «Добавить сотрудника» — одноразовая ссылка скопируется в буфер.',
                icon: '🔗',
              },
              {
                step: '3',
                title: 'Отправьте сотруднику',
                desc: 'Ссылка активна 30 минут и рассчитана строго на 1 сотрудника (одноразовая).',
                icon: '⏱️',
              },
              {
                step: '4',
                title: 'Запуск в Telegram',
                desc: 'Сотрудник нажимает «Запустить» в боте и моментально подключается к рассылке.',
                icon: '🚀',
              },
            ].map((st) => (
              <div key={st.step} className="bg-white p-3.5 rounded-r12 border border-slate-100/90 shadow-2xs space-y-1.5">
                ...
                <h4 className="text-xs font-bold text-brand-dark pt-0.5">{st.title}</h4>
                <p className="text-[11px] text-brand-gray-blue leading-relaxed">{st.desc}</p>
              </div>
            ))}
          </div>

          {/* Coffee Shop Selector & Telegram Management Panel */}
          <Card className="space-y-6">
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h4 className="text-sm font-bold text-brand-dark">Управление точками и сотрудниками</h4>
                  <p className="text-xs text-brand-gray-blue">Выберите кофейню для просмотра и добавления получателей</p>
                </div>

                {currentTgShop && (
                  <div className="flex items-center gap-2">
                    <Badge variant={tgRecipients.length > 0 ? "success" : "neutral"} size="md">
                      {tgRecipients.length > 0 ? `🟢 ${tgRecipients.length} подкл.` : '⚪️ Не подключено'}
                    </Badge>
                  </div>
                )}
              </div>

              {/* Coffee Shop Pills / Quick Selector */}
              <div className="flex flex-wrap gap-2 pt-1">
                {shops.map(s => {
                  const isSelected = (selectedTgShopId || shops[0]?.id) === s.id;
                  const recCount = s.telegram_recipients_count ?? (s.telegram_id ? 1 : 0);
                  return (
                    <button
                      key={s.id}
                      type="button"
                      onClick={() => {
                        setSelectedTgShopId(s.id);
                        setTgBindLink(null);
                        setTgBindToken(null);
                        setHasCopiedLink(false);
                        loadRecipients(s.id);
            ...
            {/* Selected Shop Telegram Details */}
            {currentTgShop ? (
              <div className="bg-slate-50/70 p-5 rounded-r18 border border-slate-200/80 space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/60">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-r12 bg-sky-500/10 text-sky-600 flex items-center justify-center font-bold text-sm shrink-0">
                      ☕
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-slate-900">
                        {currentTgShop.street}, {currentTgShop.building_number}
                      </h4>
                      <p className="text-xs text-slate-500">
                        {currentTgShop.city_name || 'Альметьевск'} • Время работы: {currentTgShop.time_open} – {currentTgShop.time_close}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {tgRecipients.length > 1 && (
                      <Button
                        ...
                        onClick={() => handleTestRecipient(undefined, currentTgShop.id)}
                  ...
                  {isLoadingRecipients ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                      {tgRecipients.map(r => (
                        <div
                          key={r.id}
                          className="bg-white p-3 rounded-r12 border border-slate-200/80 flex items-center justify-between gap-2 shadow-xs hover:border-slate-300 transition-colors"
                        >
                          <div className="flex items-center gap-3 min-w-0 pr-1">
                            <div className="w-9 h-9 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center font-bold text-xs shrink-0 select-none">
                              {r.first_name ? r.first_name[0].toUpperCase() : r.telegram_username ? r.telegram_username.replace('@', '')[0].toUpperCase() : 'TG'}
                            </div>
                            <div className="min-w-0">
                              <div className="font-semibold text-slate-900 text-xs flex items-center gap-1.5 truncate">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              type="button"
                              onClick={() => handleTestRecipient(r.telegram_id, currentTgShop.id)}
                            ...
                            <button
                              type="button"
                              onClick={() => handleDeleteRecipient(r.id, currentTgShop.id)}
                    ...
                    <div className="bg-white p-6 rounded-r12 border border-dashed border-slate-200 text-center space-y-2">
                      <div className="w-10 h-10 rounded-full bg-slate-100 text-slate-400 mx-auto flex items-center justify-center">
                        <Users className="w-5 h-5" />
                      </div>
                      <p className="text-xs font-semibold text-slate-700">В этой кофейне пока нет подключенных сотрудников</p>
                      <p className="text-[11px] text-slate-400 max-w-md mx-auto">
                        Нажмите кнопку «Добавить сотрудника через Telegram» ниже, чтобы создать пригласительную ссылку для управляющего или бариста.
                      </p>
                    </div>
                  )}
                </div>

                {/* Add Recipient Button & Link Generator */}
                <div className="space-y-3 pt-2">
                  <button
                    type="button"
                    onClick={() => handleGenerateTgLink(currentTgShop.id)}
                    disabled={isGeneratingTgLink}
                    className="w-full h-11 px-4 rounded-r12 border border-sky-200 bg-sky-50/70 hover:bg-sky-100 text-sky-700 font-bold text-xs inline-flex items-center justify-center gap-2 transition-all disabled:opacity-50 active:scale-[0.99] shadow-xs"
                  >
                  ...
                  {tgRecipients.length > 0 && (
                    <div className="flex justify-end pt-1">
                      <button
                        type="button"
                        onClick={() => handleUnlinkTg(currentTgShop.id)}
                      ...
                      >
                        ...
                        <span>{isUnlinkingTg ? 'Отвязка...' : 'Отвязать всех получателей этой точки'}</span>
                      </button>
                    </div>
                  )}
                  ...
                  <details className="group text-xs text-slate-500 pt-2 border-t border-slate-200/60">
                    ...
                    <div className="space-y-3 pt-2.5 max-w-lg">
                      <Input
                        label="Telegram ID (числовой chat_id)"
                        placeholder="Например: 6463435986"
                        value={tgManualId}
                        onChange={e => setTgManualId(e.target.value)}
                      />
                      <Input
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <p className="text-[11px] text-slate-400">
                          ID можно узнать через <a href="https://t.me/getmyid_bot" target="_blank" rel="noreferrer" className="text-sky-600 underline">@getmyid_bot</a>
                        </p>
                        <Button
                          size="sm"
                          variant="dark"
                          onClick={handleSaveManualTg}
                          isLoading={isSavingManualTg}
                          disabled={isSavingManualTg}
                        >
                          Сохранить параметры
                        </Button>
                      </div>
              ...
              <div className="text-center py-8 text-xs text-slate-400">
                Кофейни не найдены. Сначала добавьте кофейню на вкладке «Кофейни и Точки».
              </div>
            )}
          </Card>
        </div>
